...
## 出力形式
出力したMarkdownは送信直前にDiscordが解釈できる記法へ自動変換されます。
表・水平線・画像・4段以上の見出し・脚注は、通常のMarkdownのまま書いて構いません。
...
- コードブロック (\`\`\` - \`\`\`)
- 引用 (>) ※ネストは1段に潰される
- テーブル ※等幅の表、または「1レコード＝1ブロック」のラベル付き箇条書きに変換される
...
### 表を書くときの注意
横に広い表はラベル付き箇条書きへ落とされ、縦に長くなります。
列は4つ程度まで、セルは短い語句に収めてください。
